...
def main():
    ...
    for num_workers in workers_list:
        ...
        duration = run_parallel(building_ids, LOAD_DIR, MAX_ITER, ABS_TOL, num_workers)
        print(f"Time taken with {num_workers} workers: {duration:.2f} seconds")
        timings[num_workers] = duration
    overhead_time = time.time() - overhead_timer
    timings = {w: t + overhead_time for w, t in timings.items()}
    ...
    fig.tight_layout()
    plt.grid(True)
    plt.savefig("speedup_and_time_plot.png")
